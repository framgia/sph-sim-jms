...
describe("JobController", () => {
  ...
  describe("createJobWithCustomerAndSchedules", () => {
    it("should return a new job", async () => {
      ...
      const createdJob = await jobService.createJobWithCustomerAndSchedules(
        jobInput,
      );
  ...
  describe("checkIfJobAlreadyExists", () => {
    it("should return return a job", async () => {
      ...
      const checkIfJobExists = await jobService.checkIfJobAlreadyExists(
        validTitle,
        customerId,
      );
  ...
  describe("createSchedules", () => {
    it("should throw BadRequestException if schedules is an empty array", () => {
      ...
      jest
        .spyOn(jobService, "createSchedules")
        .mockRejectedValue(
          new BadRequestException("Something went wrong. Schedule not created"),
        );
  ...
  describe("findAllTypes", () => {
    ...
    it("should return string array of types", async () => {
      ...
      const returnedTypes = await jobService.findAllTypes();
  ...
  describe("findAllPersonInCharge", () => {
    ...
    const mockUserIdValues = [1, 2, 3, 4, 5];

    const mockFindManyValues = {
      where: {
        id: {
          in: mockUserIdValues,
        },
    ...
    it("should return ", async () => {
      prisma.job.groupBy = jest.fn().mockResolvedValue(mockJobValues);

      await jobService.findAllPersonInCharge();

      expect(prisma.user.findMany).toHaveBeenCalledWith(mockFindManyValues);
    });
  });
});
