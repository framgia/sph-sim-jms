import { Customer, Job, User } from "@prisma/client";
import {
  UsePipes,
  ValidationPipe,
  BadRequestException,
...
export class JobService extends AbstractService {
  ...
  async createJobWithCustomerAndSchedules(
    createJobWithCustomerAndSchedulesOptions: CreateJobWithCustomerAndSchedulesDto,
  ): Promise<Job> {
    const { customer_registration, job_information, work_schedules } =
  ...
  async checkIfJobAlreadyExists(
    ...
    if (jobAlreadyExists) {
      throw new BadRequestException(
        "Job with the same title and customer already exists.",
      );
    }
  ...
  async createCustomer(options: CreateCustomerDto): Promise<Customer> {
    ...
    if (!newCustomer) {
      throw new BadRequestException(
        "Something went wrong. Customer not created.",
      );
    }
  ...
  async createSchedules(
    schedulesData: CreateScheduleWithoutJobIdDto[],
    jobId: number,
  ): Promise<void> {
  ...
  async findAllTypes(): Promise<string[]> {
    ...
    return types.map((x) => x.type);
  }

  async findAllPersonInCharge(): Promise<User[]> {
    const userIds = await this.prisma.job.groupBy({
      by: ["userId"],
    });

    const personInChargeIds = userIds.map((x) => x.userId);

    const users = await this.prisma.user.findMany({
      where: {
        id: {
          in: personInChargeIds,
        },
      },
    });
